...
gt_folder = 'gtFine_trainvaltest/gtFine'
    ...
    for c in cities:
        ...
        for it in c_items:
            ...
            if mode == "train" and maxSkip > 0:
                new_img_path = os.path.join(aug_root, image_folder)
                new_mask_path = os.path.join(aug_root, gt_folder)
...
def make_cv_splits(img_dir_name):
    ...
    """
    trn_path = os.path.join(root, image_folder, 'train')
    val_path = os.path.join(root, image_folder, 'val')
...
def make_test_split(img_dir_name):
    test_path = os.path.join(root, img_dir_name, image_folder, 'test')
...
def make_dataset(quality, mode, maxSkip=0, fine_coarse_mult=6, cv_split=0):
    ...
    if quality == 'coarse':
        ...
        img_path = os.path.join(root, img_dir_name, image_folder, 'train_extra')
        ...
        mask_path = os.path.join(root, gt_folder)
