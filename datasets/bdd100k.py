...
def add_items(items, aug_items, img_path, mask_path, mask_postfix, mode, maxSkip):
    ...
    """
    if mode == 'night':
        with open(cfg.DATASET.BDDBIGHT_LIST, 'r') as f:
            while True:
                night_path = f.readline()
                if not night_path:
                    break 
                night_path = night_path.replace('\n', '').split('/')
                item = (os.path.join(img_path, *night_path),
                        os.path.join(mask_path, night_path[0], 
                                     night_path[1].replace(img_postfix, mask_postfix)))
                items.append(item)
        return 
...
def make_dataset(mode, maxSkip=0, cv_split=0):
    ...
    assert mode in ['train', 'val', 'test', 'trainval', 'night']
    img_dir_name = 'images'
