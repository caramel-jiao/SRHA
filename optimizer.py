...
def get_optimizer(args, net):
    ...
    """
    base_params = []
    freeze_msg = 'Freeze param: '

    for name, param in net.named_parameters():
        if name.split('.')[0] in getattr(net, 'frozen_layers', []):
            freeze_msg += '`%s`, ' % name
            continue
        base_params.append(param)
    logging.info(freeze_msg)
...
def get_encoder_decoder_optimizer(args, net):
    """
    Decide Optimizer (Adam or SGD)
    """
    encoder_params = []
    decoder_params = []
    freeze_msg = 'Freeze param: '
    encoder_names = ['layer%d' % itm for itm in range(5)]

    for name, param in net.named_parameters():
        if name.split('.')[0] in getattr(net, 'frozen_layers', []):
            freeze_msg += '`%s`, ' % name
            continue
        if name.split('.')[0] in encoder_names:
    ...
    if args.sgd:
        optimizer = optim.SGD([{'params': encoder_params},
                               {'params': decoder_params, 'lr': getattr(args, 'decoder_lr', args.lr)}],
                            lr=args.lr,
                            weight_decay=5e-4, #args.weight_decay,
                            momentum=args.momentum,
                            nesterov=False)
...
def load_pretrained_model(net, path):
    ...
    logging.info("Load Pretrained Seg Model from `%s`" % path)
    return net 
