...
def add_items(items, aug_items, cities, img_path, mask_path, mask_postfix, mode, maxSkip):
    ...
    if mode == 'val':
        mode = 'valid'
    for it in cities:
        item = (os.path.join(img_path, mode, 'folder', it),
                os.path.join(mask_path, mode, 'folder', it))
        items.append(item)
...
def make_dataset(mode, maxSkip=0, cv_split=0, combine_all=False):
    ...
    if mode != 'trainval':
        ...
        train_list_filepath = os.path.join(list_path, 'gtav_split_' + 'train' + ".txt")
        if not os.path.exists(train_list_filepath):
            raise Warning("train split not found")
        val_list_filepath = os.path.join(list_path, 'gtav_split_' + 'val' + ".txt")
        if not os.path.exists(val_list_filepath):
            raise Warning("train split not found")
        item_names = dict(train=[id.strip() for id in open(train_list_filepath)],
                          val=[id.strip() for id in open(val_list_filepath)])
    ...
    for mode_item in modes:
        ...
        add_items(items, aug_items, item_names, img_path, mask_path,
                    mask_postfix, mode_item, maxSkip)
