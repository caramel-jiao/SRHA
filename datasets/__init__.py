...
def setup_loaders(args):
    ...
    if 'gtav' in args.dataset:
        dataset = gtav
        gtav_mode = args.gtav_mode ## Can be trainval
        train_joint_transform_list, train_joint_transform = get_train_joint_transform(args, dataset)
        train_input_transform, val_input_transform = get_input_transforms(args, dataset)
        target_transform, target_train_transform, target_aux_train_transform = get_target_transforms(args, dataset)
    ...
    if 'synthia' in args.dataset:
        dataset = synthia
        synthia_mode = getattr(args, 'synthia_mode', 'train') ## Can be trainval
        train_joint_transform_list, train_joint_transform = get_train_joint_transform(args, dataset)
        train_input_transform, val_input_transform = get_input_transforms(args, dataset)
        target_transform, target_train_transform, target_aux_train_transform = get_target_transforms(args, dataset)
...
def get_train_joint_transform_imagenet(args, dataset):
    """
    Get train joint transform
    Args:
        args: input config arguments
...
def setup_imagenet_loader(args):
    dataset = imagenet
    imagenet_mode = 'train' ## Can be trainval
    train_joint_transform_list, train_joint_transform = get_train_joint_transform_imagenet(args, dataset)
    train_input_transform, val_input_transform = get_input_transforms(args, dataset)
    imagenet_set = dataset.ImageNet(
        imagenet_mode, 0,
        joint_transform=train_joint_transform,
        transform=train_input_transform,
        dump_images=False,
    ...
    if args.syncbn:
        imagenet_sampler = DistributedSampler(imagenet_set, pad=True, 
                                permutation=True, consecutive_sample=False)
    else:
        imagenet_sampler = None
    imagenet_loader = DataLoader(imagenet_set, batch_size=args.train_batch_size,
                            num_workers=args.num_workers, pin_memory=True, 
                            shuffle=(imagenet_sampler is None), drop_last=True, 
